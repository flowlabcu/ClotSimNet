...
"""
train_mlp_dend_3d_pretrain.py

This script fine-tunes a pretrained MLP model for predicting permeability from 3D clot data
using dendrogram-based image features. It performs k-fold cross-validation with selective layer
...
- Load pretrained model weights from a `.safetensors` file.
...
- Hyperparameters JSON: `ClotSimNet-Models/hyperparameters/best_params_mlp_dend.json`
- Pretrained model: `ClotSimNet-Models/tuned/mlp_dend/mlp_dend_tuned.safetensors`
- Datasets: CSV files located under `clotsimnet/data/`

Can optionally change the JSON, pretrained model, and datasets paths to wherever you have these stored.
"""
...
def main():
    ...
    BASE_RUN_NAME = 'mlp_dend_3d_pretrain_100'         # base name for WandB runs
    num_workers = 16
    max_epochs = 10
    
    hyperparameter_path = '/home/josh/ClotSimNet-Models/hyperparameters/best_params_mlp_dend.json'
    ...
    # Hyperparameters from tuned first-order model
    ...
    pretrained_model_dir = '/home/josh/ClotSimNet-Models/tuned/mlp_dend/'
    pretrained_model_name = 'mlp_dend_tuned'  # original model name (without extension)
    ...
    data_module = load_data.MLPClotDatasetDendrogram(
        csv_path=csv_path,
    ...
    test_module = load_data.MLPClotDatasetDendrogram(
        csv_path=test_path,
        num_workers=num_workers
    )
    test_module.setup()
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(features_tensor), start=1):
        ...
        # Instantiate and load model with hyperparameters from file read above
        model = MLP.MLP(
        ...
        try:
            ...
            print('Unfreezing final layer failed')
            pass
