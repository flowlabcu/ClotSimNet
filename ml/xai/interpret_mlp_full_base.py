...
# Load pretrained model using safetensors
...
SAFETENSORS_MODEL_PATH = '/home/josh/ClotSimNet-Models/base/mlp_full/mlp_full_base.safetensors'
load_safetensors(model_obj=model, model_path=SAFETENSORS_MODEL_PATH)

model.eval()

# Instantiate the three attribution algorithms
dl = DeepLift(model)
gs = GradientShap(model)
fa = FeatureAblation(model)

# Compute attributions on the test set
dl_attr_test = dl.attribute(X_test)
gs_attr_test = gs.attribute(X_test, baselines=X_train)
fa_attr_test = fa.attribute(X_test)

# Plotting of top N features
...
plt.xticks(x_axis_data + 2.5*width,
           short_labels,
           rotation=45, ha='right',
           fontweight=weight, fontsize=font_size)
plt.ylabel('Attributions', fontweight=weight, fontsize=font_size)
plt.title('Top {} Input Feature Importances Across Multiple Algorithms, Full Base MLP'.format(top_n), fontweight=weight, fontsize=font_size)
...
plt.tight_layout()
# plt.show()
plt.savefig('mlp_full_base_attribs.svg')
