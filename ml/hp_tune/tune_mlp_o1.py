...
This script tunes the first-order MLP on engineered tabular features for predicting permeability from CFD-derived clot simulations. It uses Ray Tune with the ASHA scheduler to search over hyperparameters and leverages PyTorch Lightning and Weights & Biases (W&B) for scalable training and experiment tracking.
...
- Saves best hyperparameters and validation loss to 
  `hyper_params/best_params_mlp_o1.json`.

Required:
---------
- A CSV file with feature columns matching the hardcoded `feature_list` in `MLPClotDataseto1.setup` and a `k` column for permeability.
...
def train_func(config):
    ...
    data_module = MLPClotDataseto1(csv_path=csv_path, config=config,  num_workers=num_workers)
    data_module.setup()
...
if __name__ == '__main__':
    ...
    file_name = 'best_params_mlp_o1.json'
