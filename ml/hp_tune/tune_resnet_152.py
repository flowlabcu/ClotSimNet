...
This script tunes a ResNet-18 model for predicting permeability from grayscale CFD images
...
- Uses ResNet-152 with a custom grayscale input stem and single regression output.
...
- Automatically saves best config and loss to `hyper_params/best_params_resnet_152.json`.
...
NUM_SAMPLES = 75 # Number of samples from parameter space
...
class ResNet152(L.LightningModule):
    def __init__(
        ...
        self.use_weights = True # Use pre-trained weights
        self.batch_size = config['batch_size']
...
if __name__ == '__main__':
    ...
    # Initialize Ray by first stopping any existing processes, then start it again fresh
    ...
    file_name = 'best_params_resnet_152_2.json'
