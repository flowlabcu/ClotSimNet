...
class RunFlow:
    ...
    def run_cfd(self):
        ...
        tree = FieldSplitTree(stk, split)
        solver_stokes = BlockNonLinearSolver(tree, self.mesh.comm,
        ...
        u_avg_z = fe.assemble(u[2] * stk.dx) / height
        u_avg = np.sqrt(u_avg_x**2 + u_avg_y**2 + u_avg_z**2)

        delta_p = (fe.assemble(p * stk.ds(id_out)) - fe.assemble(p * stk.ds(id_in))) / width**2
        delta_x = length
