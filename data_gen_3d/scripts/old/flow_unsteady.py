...
class RunFlow:
    ...
    def run_cfd(self):
        ...
        # Define problem object (Unsteady incomp. Navier Stokes)
        nse_un = IncompressibleNavierStokes(self.mesh)
        nse_un.set_element('CG', 1, 'CG', 1)
        nse_un.build_function_space()
        ...
        # For unsteady case
        nse_un.set_dynamic_viscosity(mu)
        nse_un.set_density(rho)
        nse_un.set_time_step_size(0.1)
        nse_un.set_mid_point_theta(0.5)
        nse_un.set_weak_form()
        nse_un.add_stab()
        ...
        # Set solver and solve
        problem_un = NonLinearProblem(nse_un)
        tree_un = FieldSplitTree(nse_un, split)
        solver_un = BlockNonLinearSolver(tree_un, self.mesh.comm,
                                    problem_un, fe.PETScKrylovSolver(),
                                    outer_ksp_set_function=self.set_outer_ksp)

        solver_un.solve()
        
        (u0, p0) = fe.split(nse_un.solution)
        
        nse.set_initial_guess(initial_guess_u=u0, initial_guess_p=p0)
