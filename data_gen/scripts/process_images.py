...
def process_image(
    ...
    """
    Extracts image features from a cropped CFD output image and appends them to the simulation's data CSV file.
    
    This function computes both first-order (statistical intensity-based) and second-order (texture-based GLCM)
    features from the input image. It also applies Laws' texture energy masks to extract convolutional features.
    The resulting features are appended as a new row to the simulation's `data.csv`.
    
    Parameters:
        image_path (str): Path to the image.
        case_dir (str): Path to the specific CFD simulation. Must contain `data.csv`.
        
    Returns:
        None
        
    Notes:
        - Assumes `data.csv` exists in `case_dir` and contains one row of simulation metadata.
        - The image is converted to grayscale and 8-bit format before processing.
        - Output CSV will include original simulation data plus image features as additional columns.
    """
